...
import { 
  ...
  XCircle, AlertTriangle, ChevronLeft, ChevronRight, Filter as FilterIcon
} from 'lucide-react';
...
const EventsNews: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (savedEvents) {
      ...
      // Set initial events if no storage exists
    ...
    <div className="bg-gray-50 min-h-screen">
      <div className="bg-primary py-24 px-4 text-center relative overflow-hidden">
        ...
        <div className="max-w-4xl mx-auto relative z-10">
          <h1 className="text-5xl lg:text-7xl font-bold text-white mb-6 font-serif tracking-tight">Events & News</h1>
          <p className="text-xl text-blue-100 italic max-w-2xl mx-auto font-medium">
            Stay updated with our latest activities, professional seminars, and fellowship gatherings across the Republic.
          </p>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 py-16">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
          ...
          <div className="lg:col-span-8 space-y-12">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between border-b border-gray-200 pb-6 gap-4">
              <div>
                ...
                <p className="text-sm text-gray-500 mt-1">Book your seat for seminars, conventions and fellowships</p>
              </div>
            </div>
            ...
            <div className="space-y-8">
              {filteredEvents.map((event) => {
                ...
                const capacityPercent = (event.filled / event.capacity) * 100;
          ...
          <div className="lg:col-span-4 space-y-12">
            <div className="space-y-8">
              <h2 className="text-2xl font-black text-gray-900 flex items-center font-serif border-b border-gray-200 pb-6">
                <Newspaper className="mr-3 text-secondary" size={24} /> Regional Bulletins
              </h2>
              <div className="space-y-6">
                {NEWS_ARTICLES.map((article) => (
                  <div key={article.id} className="bg-white rounded-3xl overflow-hidden shadow-sm border border-gray-100 hover:shadow-md transition-all group">
                    <div className="h-44 overflow-hidden">
                      <img src={article.image} alt={article.title} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500" />
                    </div>
                    <div className="p-6">
                      <h3 className="font-black text-gray-900 mb-3 text-base leading-tight hover:text-primary transition-colors cursor-pointer">{article.title}</h3>
                      <p className="text-[10px] font-black text-primary uppercase tracking-widest">{article.date}</p>
                    </div>
                  </div>
                ))}
              </div>
